...
class TechController:
    ...
    @staticmethod
    def get_available_tickets(db: Session, technician: User, skip: int = 0, limit: int = 100) -> List[TicketResponse]:
        """Obtém tickets disponíveis (atribuídos + não atribuídos)"""
        tickets = TicketService.get_available_tickets_for_technician(db, technician.id, skip, limit)
        return [TicketResponse.from_orm(ticket) for ticket in tickets]

    @staticmethod
    def get_unassigned_tickets(db: Session, skip: int = 0, limit: int = 100) -> List[TicketResponse]:
        """Obtém tickets não atribuídos (disponíveis para pegar)"""
        tickets = TicketService.get_unassigned_tickets(db, skip, limit)
        return [TicketResponse.from_orm(ticket) for ticket in tickets]

    @staticmethod
    def take_ticket(db: Session, ticket_id: int, technician: User) -> TicketResponse:
        """Permite que técnico pegue um ticket não atribuído"""
        ticket = TicketService.assign_ticket_to_self(db, ticket_id, technician.id)
        if not ticket:
