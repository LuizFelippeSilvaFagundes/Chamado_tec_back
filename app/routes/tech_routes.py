...
def get_tech_tickets(skip: int = 0, limit: int = 100, current_user: User = Depends(require_technician_or_admin), db: Session = Depends(get_db)):
    ...
    return TechController.get_available_tickets(db, current_user, skip, limit)

@router.get("/tickets/assigned", response_model=List[TicketResponse])
def get_assigned_tickets(skip: int = 0, limit: int = 100, current_user: User = Depends(require_technician_or_admin), db: Session = Depends(get_db)):
...
def get_available_tickets(skip: int = 0, limit: int = 100, current_user: User = Depends(require_technician_or_admin), db: Session = Depends(get_db)):
    """Obter tickets não atribuídos (disponíveis para pegar)"""
    return TechController.get_unassigned_tickets(db, skip, limit)
...
def take_ticket(ticket_id: int, current_user: User = Depends(require_technician_or_admin), db: Session = Depends(get_db)):
    """Pegar um ticket não atribuído"""
    return TechController.take_ticket(db, ticket_id, current_user)
